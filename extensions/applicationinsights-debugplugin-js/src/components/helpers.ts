    ...
    if (isObj) {
      rootDiv.onclick = (evt: MouseEvent) => {
        ...
        if (s.lastSelectedElement) {
          const cn = s.lastSelectedElement.className;
          const i = s.lastSelectedElement.className.indexOf(' last-selected-element');
          s.lastSelectedElement.className = cn.substring(0, i) + cn.substring(i + 22);
        }
        s.lastSelectedElement = (evt.target as HTMLElement);
        for (let i = 0; i < 10; i++) {
